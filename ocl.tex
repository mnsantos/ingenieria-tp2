...
\oclInv{Alquiler}{self.Penalizacion $\rightarrow$ forAll(p | (p.Causa = Desaparicion\_bicicleta) implies (self.bicicleta.estaEn $\rightarrow$ size() = 0 $\wedge$ self.bicicleta.pedido $\rightarrow$ size() = 0))}{Si una bicicleta desaparece no aparecerá en ningún pedido o alquiler futuro}
