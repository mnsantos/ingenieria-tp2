...
\oclInv{Pedido}{((self.EstacionDestino.empleados $\rightarrow$ select(e | e.id = self.responsabilidadDe.id)) $\rightarrow$ size()) = \ 1}{Los pedidos tienen que tener como responsable a un empleado que trabaje en la estacion destino}
~
